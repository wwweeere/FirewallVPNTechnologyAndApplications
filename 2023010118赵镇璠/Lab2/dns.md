...
| 程序输出的 IP 地址 |140.143.177.206、81.70.124.99、120.53.130.158|
...
| 回答记录数（Answers）        |    5       |
| 返回的 IP 地址               |140.143.177.206、81.70.124.99、120.53.130.158|
| TTL（若可见）                |第一条 CNAME 记录（www.douban.com）的 TTL 为 202 秒其余 4 条记录（1 条 CNAME + 3 条 A 记录）的 TTL 均为 17 秒|
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：作用：DNS（域名系统）的核心作用是将人类可读的域名（如 www.douban.com）转换为网络设备可识别的 IP 地址（如 140.143.177.206），实现域名与 IP 地址的映射。
原因：计算机网络通信依赖 IP 地址定位设备，而人类难以记忆纯数字的 IP 地址。DNS 解析充当 “翻译官”，让用户通过易记的域名访问网站，同时完成底层网络路由所需的 IP 地址转换。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致。作用：该字段是 DNS 事务的唯一标识，用于匹配请求与响应，确保客户端能将收到的响应正确对应到之前发出的查询请求，避免多并发查询时的数据包混淆。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录（Address Record）是 DNS 中最基础的记录类型，用于将域名直接映射到一个 IPv4 地址，是客户端获取目标网站 IPv4 地址的核心查询类型。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认配置：DNS 默认使用 UDP 协议，端口为 53。
改用 TCP 的场景：
（1）当 DNS 响应数据包大小超过 512 字节时（UDP 报文长度限制），会触发 TCP 传输；
（2）区域传送（Zone Transfer）操作（主从 DNS 服务器同步域名数据）必须使用 TCP；
（3）开启 DNSSEC（安全扩展）时，为保证数据完整性与安全性，会使用 TCP。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：（1）不支持 IPv6：gethostbyname() 仅能解析 IPv4 地址，无法处理 IPv6 网络环境，不符合现代网络协议演进趋势；
（2）线程不安全：该接口返回的静态内存缓冲区在多线程场景下可能被覆盖，导致数据竞争与解析结果错乱；
（3）功能局限：无法获取完整的 DNS 解析信息（如 TTL、记录类型等），现代替代接口（如 getaddrinfo()）提供了更灵活、全面的解析能力。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：说明该网站采用了负载均衡策略，通过将流量分发到多个服务器，提升服务可用性与并发处理能力；也可能是CDN（内容分发网络） 部署的体现，多个 IP 对应不同地域的边缘节点，让用户访问最近的服务器以降低延迟；
部分场景下也可能是高可用架构的实现，避免单点故障。
