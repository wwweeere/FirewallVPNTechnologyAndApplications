...
| 测试 | 预期 | 你的结果 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |FORWARD 链第 2 条：允许 10.1.0.0/24 访问 10.2.0.2 tcp:8080 新建连接 |
| `siteA -> siteB:2222` | 成功 |先匹配 LOG 日志规则，再匹配 REJECT 拒绝规则 |
| `siteA -> siteB ping` | 成功 |先匹配 LOG 日志规则，再匹配 REJECT 拒绝规则 |
...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |仅允许 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接 |A-TO-B-DENY |DROP |
| `gwB` |仅允许 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接 |VPN-S2S-DENY |DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |正常返回网页目录 |无拒绝日志，流量放行 |
| `siteA -> siteB:2222` | 失败 |curl 连接超时失败 |生成A-TO-B-DENY TCP 拦截日志 |
| `siteA -> siteB ping` | 失败 |全部 ICMP 不可达，100% 丢包 |生成A-TO-B-DENY ICMP 拦截日志 |
...
| underlay 是否连通 |连通，gwA 可 ping 通 gwB 192.0.2.2 |
...
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功 |FORWARD 链第 2 条：允许 10.1.0.0/24 访问 10.2.0.2 tcp:8080 新建连接 |
| `siteA -> siteB:2222` |失败 |先匹配 LOG 日志规则，再匹配 REJECT 拒绝规则 |
| `siteA -> siteB ping` |失败 |先匹配 LOG 日志规则，再匹配 REJECT 拒绝规则 |

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1 ↔ 192.0.2.2 |UDP 51820 |WireGuard 外层加密封装报文，内网 IP 被加密隐藏 |
| `wg0` |10.1.0.2 ↔ 10.2.0.2 |TCP 8080 |解密后的原始业务内网流量，可见真实私有 IP 与业务端口 |
| B 站点 LAN |10.1.0.2 ↔ 10.2.0.2 |TCP 8080 |经过防火墙放行、转发到 B 站点内网的原始业务报文 |
...
业务场景
远程接入：单个移动 / 远程客户端接入企业单一内网，一对多；
站点到站点：两个完整私有网段（总部 / 分支 / 云 VPC）双向互通，网段对网段。
AllowedIPs 配置
远程接入：客户端写企业内网网段，网关写客户端隧道单 IP；
站点到站点：两端网关 Peer 均填写对方完整内网网段。
路由逻辑
远程接入：仅客户端单向访问内网，回程路由由网关自动维护；
站点到站点：两端双向路由，两个网段任意主机可互访。
部署对象
远程接入：客户端设备 + 中心网关；
站点到站点：两端站点出口网关，无终端客户端参与隧道建立。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
WireGuard 的 AllowedIPs 具备两大核心作用：路由分发 + 入站安全校验
路由分发：当网关收到发往对端内网（10.2.0.0/24）的数据包，自动匹配该 Peer，封装加密并通过隧道转发；wg-quick 会自动生成对应网段路由指向 wg0 接口。
入站校验：隧道解密后，校验内层数据包源 IP 是否匹配AllowedIPs网段；不匹配则直接丢弃，防止伪造内网 IP 的恶意流量穿透隧道。
站点互通需要整段内网流量走隧道，因此必须填写完整对端内网网段，而非单个 IP。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
网关需要完成跨三层接口数据包转发：
gwA：内网 veth-gwa-lan 接收 siteA 流量，转发到 wg0 隧道接口；
gwB：wg0 隧道解密流量，转发到内网 veth-gwb-lan 发给 siteB。
Linux 内核默认关闭 IPv4 转发（仅本机进程收发报文），ip_forward=1开启跨网卡三层转发，不开启则跨网段流量会被内核直接丢弃，隧道握手正常但业务不通。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
siteA/siteB 主机仅拥有本地内网直连路由，没有对端 10.x.x.x 网段的静态路由：
主机访问异地网段时，会匹配默认路由将数据包转发给本地网关 gwA/gwB；再由网关通过 WireGuard 隧道完成跨站点转发。
若不配置该默认网关，主机无法将跨站点流量送达 VPN 网关，直接提示网络不可达。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
检查两端网关net.ipv4.ip_forward=1是否开启；
核对两端 Peer 的AllowedIPs网段是否正确匹配对端内网；
查看网关路由表，确认对端内网网段路由是否指向 wg0；
检查 iptables FORWARD 链默认策略、放行规则是否拦截跨站点流量；
检查 siteA/siteB 主机默认网关是否正确指向本地网关；
排查 MTU 值，隧道默认 MTU1420，过大易产生分片丢包；
抓包验证：underlay 是否有加密 UDP 包、wg0 是否有解密内网报文。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
双向配置好处（纵深防御）
双重防护：单侧网关配置错误时，另一侧仍能拦截非法流量，保护内网；
流量分层管控：gwA 管控出站流量，gwB 管控入站流量，审计日志分离，故障定位清晰；
最小化流量损耗：非法流量在 gwA 直接拦截，无需加密传输占用 underlay 带宽。
仅单侧配置缺陷
不推荐只写一侧：
仅 gwA 配置：若 gwA 规则失效，非法流量会完整加密传输到 gwB 内网，无二次拦截；
仅 gwB 配置：非法流量全部经过隧道加密传输，浪费带宽，增加安全暴露面。
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
隐藏的信息（加密内层载荷）
两端内网真实私有 IP（10.1.0.2/10.2.0.2）；
业务层 TCP/UDP 端口（8080/2222/ICMP）；
HTTP 明文业务数据、TCP 握手、ICMP 报文内容；
内网主机之间的通信行为。
暴露的信息（外层 Underlay 封装头）
两端网关公网 / 底层 IP（192.0.2.1、192.0.2.2）；
WireGuard 固定 UDP 端口 51820；
外层 UDP 数据包长度、报文收发时间；
隧道持续保活流量特征，可识别正在使用 WireGuard。
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
WireGuard 配置无需修改
gwB 出站防火墙新增规则（B→A 隧道出口）
gwA 入站防火墙新增规则（隧道进入 A 内网）
