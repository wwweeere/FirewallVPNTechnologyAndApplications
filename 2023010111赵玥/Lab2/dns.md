...
| 字段                         | 你的截图中的值 |
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    | 0x649c               |
| 查询域名（Query Name）       |www.baidu.com                |
| 查询类型（Type）             |  	A (IPv4 Address)              |
| 目的端口                     |   53             |
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS（域名系统）的核心作用是实现域名与 IP 地址的相互映射。访问网站时先进行 DNS 解析，是因为网络设备通信依赖 IP 地址 定位目标主机，而用户更易记忆域名，DNS 解析可将人类易读的域名转换为设备可识别的 IP 地址，完成后续网络通信。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致（均为 0x649c）。该字段用于匹配 DNS 请求与对应的响应，避免不同请求的响应混淆，确保客户端识别并接收对应查询的结果。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录（Address Record）是 DNS 核心资源记录类型，含义是将主机域名映射到 IPv4 地址，是最常用的地址解析记录。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认使用 UDP 协议，端口为 53。
改用 TCP 的场景：① 响应数据长度超过 UDP 限制（512 字节）；② 进行区域传输（主从 DNS 服务器同步数据）；③ 某些安全或加密场景（如 DNS over TCP）。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：① 仅支持 IPv4，无法解析 IPv6 地址，不满足现代网络需求；② 是阻塞式接口，查询时会阻塞进程，无法异步处理，影响并发性能；③ 缺乏线程安全机制，且已被现代标准（如 getaddrinfo）废弃。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：通常表示该域名配置了负载均衡策略或多服务器集群：① 服务器通过多 IP 分发请求，提升系统吞吐量与可用性；② 可能存在多数据中心 / 地域节点，解析时返回不同 IP 以实现就近访问。
