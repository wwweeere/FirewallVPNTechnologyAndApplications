...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS的核心作用是将人类易记的域名解析为网络设备可识别的 IP 地址，是互联网的 “地址簿”。访问网站时必须先解析，是因为：网络通信依赖 IP 地址建立连接，设备无法直接识别域名；DNS 解析是访问网站的第一步，只有拿到目标服务器的 IP 地址，浏览器才能发起后续的 HTTP 请求。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：本次抓包中，请求包和响应包的 Transaction ID 完全一致，均为 0xf1aa。该字段的作用是：作为 DNS 事务的唯一标识，用于匹配同一请求对应的响应；客户端可通过该 ID 区分不同并发的 DNS 查询，避免多个查询的响应发生混淆。


3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录是最常见的 DNS 记录类型，作用是将域名映射到对应的 IPv4 地址，也就是本次实验中 “域名→IP” 的解析结果类型。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认使用：UDP 协议 + 53 端口.改用 TCP 的场景有：DNS 响应报文长度超过 512 字节，UDP 无法传输，需要 TCP 的分片能力；进行区域传输时，需要 TCP 的可靠传输保障数据完整；使用 DNSSEC 等扩展协议时，报文长度增加，也会改用 TCP。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：该接口仅能解析 IPv4 地址，无法处理现代网络中广泛使用的 IPv6，已无法满足双栈网络的需求；线程不安全：gethostbyname() 使用静态缓冲区存储结果，在多线程环境下会引发数据竞争，导致结果被意外覆盖
