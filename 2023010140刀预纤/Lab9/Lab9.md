...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |成功| curl 命令返回了完整的 HTML 目录列表，DMZ 服务器日志收到了 GET 请求。|
| `guest -> dmz:8080` |失败 | curl 命令提示 Failed to connect to server，连接被拒绝。|
| `guest -> office` |失败 |curl 命令提示 Failed to connect to server，连接被拒绝。 |
...
> 答：不会被命中。
iptables 规则是按从上到下的顺序依次匹配的，数据包一旦匹配到 REJECT 规则，就会被直接拒绝丢弃，不会再继续往下匹配后续的任何规则。因此，REJECT 规则后面的 LOG 规则永远不会被触发，日志也就无法记录。
这也是我们在配置时，必须把 LOG 规则写在 REJECT 前面的根本原因。
...
```text
May 21 10:09:31 dyqqqqqqxxxxxxx kernel: GUEST-TO-DMZ: IN=veth-fw-guest OUT=veth-fw-dmz MAC=5e:17:ce:6d:46:95:96:8b:81:c4:a7:7f:08:00 SRC=10.0.2.2 DST=10.0.3.2 LEN=60 TOS=0x00 PREC=0x00 TTL=63 ID=57302 DF PROTO=TCP SPT=56740 DPT=8080 WINDOW=64240 RES=0x00 SYN URGP=0
```
...
> 答：合理。 因为我们当前的规则只对被拒绝的流量（guest 网段）配置了 LOG 目标，而对被放行的 office 网段流量没有配置日志规则。
如果想记录所有被放行的流量，需要在对应的 ACCEPT 规则之前，添加一条匹配条件完全相同的 LOG 规则。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：可以还原。
日志中包含了关键信息：
SRC=10.0.2.2：源 IP（访客主机）
DST=10.0.3.2：目的 IP（DMZ 服务器）
DPT=8080：目的端口（HTTP 服务）
因此，可以还原出：IP 为 10.0.2.2 的 guest 主机，正在访问 10.0.3.2 服务器的 8080 端口的 HTTP 服务。
...
> 答：分类统计：可以快速统计不同类型的访问事件（如 DMZ 区被访问、办公区被访问），便于安全审计和趋势分析。
快速排查：在发生安全事件时，能通过日志前缀快速定位相关记录，大幅提高故障排查效率。
告警与监控：可以基于不同的日志前缀配置不同的告警规则，比如针对 DMZ 区的异常访问触发告警。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：日志风暴：会瞬间产生大量日志，导致日志文件迅速膨胀，耗尽磁盘空间。
系统性能下降：大量的日志写入会占用大量 CPU 和 I/O 资源，影响防火墙本身的转发性能，甚至导致系统瘫痪。
关键信息被淹没：过多的无效日志会淹没真正需要关注的安全事件，导致审计人员无法及时发现真正的攻击行为。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 |5 |
| `--limit-burst 5` 的含义 |允许一次性最多写入 5 条日志，后续日志按--limit设置的速率写入，超出部分只拦截不记录 |
...
> 答：LOG 和 REJECT 是两个相互独立、互不影响的规则。LOG 规则的作用只是记录日志，它不会改变数据包的命运；而真正决定数据包被丢弃的是 REJECT 规则。--limit 仅限制了 LOG 规则的匹配频率，不会影响后续 REJECT 规则对每个数据包的处理。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：有本质区别。前者是数据包未被处理，已进入内网，属严重漏洞；后者是数据包已被拦截，仅为防日志风暴而未记录。安全审计中混淆二者会误判安全状态，忽视潜在威胁，故不能仅凭日志判断。
...
| 2 | ACCEPT|10.0.1.0/24 -> 10.0.3.2:8080 | -|
| 3 | LOG|10.0.2.0/24 -> 10.0.3.2:8080| GUEST-TO-DMZ:|
| 4 |REJECT |10.0.2.0/24 -> 10.0.3.2:8080 | -|
| 5 | LOG| 10.0.2.0/24 -> 10.0.1.0/24|GUEST-TO-OFFICE: |
| 6 | REJECT|10.0.2.0/24 -> 10.0.1.0/24 | -|
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：为了实现安全隔离。DMZ 区存放对外提供服务的主机，更容易成为攻击目标；而 Office 区存放内部业务数据，安全等级要求更高。本实验中，防火墙规则实现了 “允许 office 访问 dmz，但拒绝 guest 访问 office” 的控制效果。如果二者在同一网段，一旦 DMZ 的服务被攻破，攻击者就能直接访问同一网段的 Office 主机，内网安全将彻底暴露。

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：Guest 网段通常是开放给外部访客或不可信设备的网络，安全等级低，存在被攻击、感染恶意软件的风险。如果不隔离，一旦访客设备被攻陷，攻击者就可以利用它作为跳板，攻击办公内网，窃取公司机密或横向移动。本实验中，防火墙规则直接拒绝了所有 guest 到 office 的访问请求，就是为了防止这类风险。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：为了让正常的访问请求能够收到回应。例如，当 office 主机主动访问 dmz 的服务时，服务器的回应数据包属于 “已建立或相关的连接”。如果去掉这条规则，回应数据包会被默认的 DROP 策略丢弃，导致 TCP 连接无法建立，office 到 dmz 的访问也会失败。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：本质区别是：LOG 是非终结性目标，它只记录日志，数据包会继续匹配后续规则；而 ACCEPT/DROP/REJECT 是终结性目标，会直接决定数据包的命运。
因此，LOG 必须放在 REJECT 前面。如果 LOG 在后面，数据包匹配到 REJECT 后会被直接丢弃，LOG 规则永远不会被触发，日志也就无法记录。

5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：这是合理的。因为我们的日志规则只配置在了拒绝流量的规则上，目的是记录和审计攻击或异常访问行为。对于被放行的合法访问，默认不记录日志可以减少系统开销。这不是疏漏，而是一种常见的 “按需审计” 策略，即重点监控和记录异常行为。

6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：有本质区别。
   “只要不记录就当没发生” 意味着数据包没有被防火墙处理，成功进入了内网，这是严重的安全漏洞。
   “仍然拦截但不记录” 意味着数据包已经被安全地拒绝了，只是为了防止日志风暴而限制了日志写入。
   安全审计中必须区分这两种情况，否则会误判安全状态，忽视潜在威胁。

7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：VPN 用户应该先接入 DMZ 区。
   网络分区角度：VPN 用户属于外部接入用户，信任等级低，不应直接接入高安全等级的 Office 内网。接入 DMZ 区可以先进行身份验证和安全检查，再由 DMZ 区通过严格的规则控制对内网的访问。
   日志审计角度：所有 VPN 用户的访问行为都可以在 DMZ 区进行统一的日志记录和审计，方便后续的安全分析和问题排查，避免直接在内网留下无法溯源的访问记录。
