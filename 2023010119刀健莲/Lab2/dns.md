...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS（域名系统）的核心作用是将人类易记的域名转换为计算机可识别的 IP 地址，它就像互联网的 “地址簿”，建立域名与 IP 地址的映射关系。
访问网站时需要先进行 DNS 解析，是因为计算机在网络中只能通过 IP 地址定位服务器，而域名只是为了方便用户记忆；DNS 解析就是把域名翻译成对应的 IP 地址，让设备能找到目标服务器并建立连接，同时它还支持 IP 地址变更后用户仍可通过原域名访问，保障服务的稳定性。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：在本次抓包中，DNS 请求包与响应包的 Transaction ID 是完全一致的（均为 0x2415），该字段由客户端随机生成，作用是作为唯一标识，将响应报文与对应的请求报文进行一一匹配，确保多并发查询时不会混淆，并辅助校验报文合法性，防范伪造的 DNS 响应攻击。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：在本次实验中，查询类型为 A 记录，其含义是：用于将主机名（域名）解析为对应的 IPv4 地址。
A（Address）记录是 DNS 中最基础的资源记录类型，它建立了域名到 IPv4 地址的一一映射关系。本次实验中，客户端向 DNS 服务器查询 www.yxnu.edu.cn 的 A 记录，服务器返回的响应结果中，最终解析到的 IP 地址 61.166.32.242 就是该域名对应的 IPv4 网络地址，这也是计算机能够在网络中定位并访问目标服务器的关键依据。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS 默认情况下主要使用 UDP 协议，端口号为 53；当 DNS 查询或响应报文长度超过 512 字节（例如包含较多资源记录、使用 DNSSEC 扩展），或是 DNS 服务器之间进行区域数据同步（Zone Transfer）时，会改用 TCP 协议，端口号同样为 53，以此保证数据可靠、完整地传输。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：gethostbyname() 不被现代工程推荐使用，主要原因包括：
仅支持 IPv4：该接口只能返回 IPv4 地址，无法获取 IPv6 地址，难以满足当前网络对 IPv6 的支持需求。
线程不安全且不可重入：使用静态缓冲区存储结果，多线程并发调用时会出现数据竞争，导致结果错误，不适合现代多线程应用场景。
已被标准废弃：该接口属于早期 BSD 套接字接口，已被现代标准（如 POSIX）废弃，替代接口更稳定、兼容性更强。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：一个域名返回多个 IP 地址，通常说明该服务采用了负载均衡、多节点部署或 CDN 加速，目的是实现流量分发、提高访问速度、增强服务可用性与冗余备份，避免单点故障并提升整体稳定性。
