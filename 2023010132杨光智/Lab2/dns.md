...
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    |       0x39e4         |
| 返回码（Response Code）      |       0 (No error)         |
| 回答记录数（Answers）        |      1         |
| 返回的 IP 地址               |      www.baidu.com           |
| TTL（若可见）                |        300        |
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS 的核心作用是将人类易记的域名转换为计算机网络通信可识别的 IP 地址，同时也支持反向的 IP 到域名的解析、邮件服务器解析等附加功能。访问网站必须先进行 DNS 解析，是因为网络层的通信只能通过 IP 地址定位目标主机，计算机无法直接通过域名建立网络连接，必须先通过 DNS 解析获取到域名对应的 IP 地址，才能和目标服务器建立 TCP/IP 连接、访问网站内容。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：我抓到的请求包和响应包中的 Transaction ID 是完全一致的。这个字段的核心作用是标识一次 DNS 查询的请求与响应的对应关系，客户端发起请求时生成一个随机的 Transaction ID，服务器返回响应时必须携带相同的 ID，客户端通过这个 ID 就能把响应和对应的请求匹配上，避免多个并发 DNS 查询时出现响应混淆的问题，同时也能一定程度上抵御 DNS 欺骗攻击。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录全称是 Address 记录，是 DNS 中最基础、最常用的记录类型，作用是将一个域名直接映射到对应的 IPv4 地址，是实现域名到 IPv4 地址解析的核心记录。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS 默认使用 UDP 传输层协议，端口号为 53。在以下两种场景会改用 TCP 协议：1. 当 DNS 响应包的大小超过 512 字节（UDP 协议的传统 MTU 限制），无法在单个 UDP 包中完整传输时，会改用 TCP 协议进行传输；2. 当进行 DNS 区域传输（AXFR），也就是主 DNS 服务器向从 DNS 服务器同步完整的域名解析记录时，必须使用 TCP 协议保证数据传输的可靠性和完整性。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：第一，该接口不支持 IPv6 地址的解析，只能处理 IPv4 地址，无法适配现代广泛部署的 IPv6 网络环境，兼容性存在严重缺陷；第二，该接口不是线程安全的，在多线程程序中并发调用时，会出现数据覆盖、结果错乱的问题，无法满足现代多线程服务的开发需求；除此之外，该接口的错误处理机制不完善，无法精准区分不同类型的解析错误，也是不推荐使用的重要原因。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：这通常说明该域名对应的服务做了负载均衡部署，通过一个域名对应多台服务器的多个 IP 地址，将用户的访问请求分散到不同的服务器上处理，以此提升服务的并发承载能力和可用性，避免单台服务器故障导致服务不可用；同时也可能是该服务部署了多地域的节点，通过 DNS 解析给用户返回就近的服务器 IP，实现就近访问、降低访问延迟。
