...
1. HTTP 请求报文由哪几部分构成？请按顺序列出：
   ...
   > 答：HTTP 请求报文由 请求行（Request Line）、请求头（Request Headers）、空行（Blank Line）、请求体（Request Body） 四部分构成，顺序固定。

2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？

   > 答：404 = 资源找不到；500 = 服务器代码报错（未知错误）；503 = 服务器暂时不可用（过载 / 维护）

3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：GET 参数在 URL 里、短小、可缓存（查数据）；POST 参数在请求体、安全、可传大文件（提交数据）

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：HTTP 明文、不安全、默认 80 端口；HTTPS 加密、安全、默认 443 端口（加了 SSL/TLS 证书）

5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？

   > 答：开发者工具读的是浏览器本地的明文，HTTPS 只加密 “网络传输” 这一段，本地查看不影响安全性。
