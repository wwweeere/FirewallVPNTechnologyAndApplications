...
| `ip route` 中 wg0 相关的条目 |0.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1     10.20.0.0/24 dev wg0 scope link         |
...
答：握手使用底层 Underlay UDP (192.0.2.x) 通信，不受 AllowedIPs 控制；业务流量依靠路由 + AllowedIPs 转发，二者路径分离，因此隧道握手正常但业务不通。
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             | 192.0.0.2                          |10.10.10.1 |
| 看到的目的地址           |192.0.2.1	                         |10.20.0.2  |
| 协议                     | UDP 51820                         |ICMP       |
| 是否能看到 ICMP/HTTP 内容 |无                                 | 明文 ICMP 请求 / HTTP 报文   |
| 为什么内外两层看到的不同 | 外层是 WireGuard 加密封装后的 UDP 包，内层原始 IP 报文被加密封装；wg0 是解密后原始数据包    |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
答：信封比喻：underlay 抓到的是外层信封（信封地址是公网 IP）；wg0 抓到的是信封里的信纸（真实内网 IP + 数据）；公网链路只能拿到加密信封，第三方窃听无法获取真实通信地址与报文，实现传输加密隐私保护。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：出口路由规则：wg-quick 自动生成路由，目的 IP 匹配网段→数据包送入 wg0 走 VPN 隧道；入口白名单校验：对端解密后内层报文源 IP 不在 AllowedIPs 网段直接丢弃。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：隧道握手依靠底层 Underlay 公网 IP+UDP 端口（Endpoint）完成密钥协商，和隧道内网网段、AllowedIPs 无关；业务数据依赖路由 + AllowedIPs 路由转发 + 入站白名单，两套寻址体系隔离，握手正常不代表流量可达。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：本机所有流量全部强制走 WireGuard 隧道，全量上网流量被 VPN 接管；VPN 网关可以拦截、监控全部客户端流量，内网泄露、路由劫持风险极大。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：WireGuard 双向公钥鉴权：vpn 配置里写入客户端公钥，只有持有对应客户端私钥的设备才能签名报文完成身份校验；仅有服务端私钥不能伪造客户端签名，无法通过认证建立隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：192.0.2.2（客户端底层物理地址）；WireGuard 将原始 IP 数据包整体加密后封装进全新 UDP 报文，内层 IP、端口、应用数据全部加密，链路抓包只能看见外层 UDP，无法解密读取内层信息。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：内容加密：内层报文全密文传输，中间人无法窃取业务数据；地址隐匿：公网链路看不到真实内网 IP，隐藏通信双方内网拓扑、访问目标。
