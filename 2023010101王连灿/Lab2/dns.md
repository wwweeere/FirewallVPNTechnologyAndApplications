...
| 字段                         | 你的截图中的值 |
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    | 0x448b               |
| 查询域名（Query Name）       | www.yxnu.edu.cn               |
| 查询类型（Type）             | A（IPv4 地址记录）               |
| 目的端口                     | 53               |
...
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    |0x448b                |
| 返回码（Response Code）      |0（No error，无错误）                |
| 回答记录数（Answers）        | 1               |
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS（域名系统）的核心作用是将人类易于记忆的域名（如 www.yxnu.edu.cn）转换为计算机能够识别的 IP 地址（如 202.203.86.93）。网络通信依赖 IP 地址进行寻址，计算机无法直接通过域名定位服务器，因此访问网站前必须通过 DNS 解析获取目标 IP 地址，才能建立连接。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：是一致的，本次实验中两个包的 ID 均为 0x448b。Transaction ID 是 DNS 事务标识符，用于匹配请求和响应，确保客户端能将收到的响应与之前发送的请求一一对应，避免收到的响应被错误分配给其他请求。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录是 DNS 的地址记录（Address Record），用于将域名映射到对应的 IPv4 地址，是最基础、最常用的 DNS 记录类型。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS 默认使用 UDP 协议，端口为 53。当 DNS 响应数据长度超过 512 字节、需要进行区域传输（Zone Transfer）或使用 DNSSEC 进行安全验证时，会改用 TCP 协议以保证数据可靠传输。


5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：① 不支持 IPv6 协议，仅能解析 IPv4 地址，无法适配当前 IPv6 网络环境；② 线程不安全，在多线程程序中调用可能导致数据竞争，引发程序异常；③ 该函数已被 POSIX 标准标记为废弃，现代应用推荐使用更安全、功能更完善的 getaddrinfo() 接口。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：这通常说明该域名对应的服务使用了负载均衡或多服务器集群部署，多个 IP 地址对应不同的后端服务器，可用于分散用户请求压力、提高服务可用性和访问性能，同时也支持基于地理位置的就近接入。
