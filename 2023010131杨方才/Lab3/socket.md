...
# 阶段 1：创建套接字
client = socket.socket(__socket.AF_INET______, ___socket.SOCK_STREAM_____)

# 阶段 2：连接服务器
client.__connect______(('127.0.0.1', 8080))

# 阶段 3：发送数据
client.__send______('你好，服务器'.encode())

# 阶段 3：接收响应
data = client._recv(1024)_______(1024)
print('收到：', data.decode())

# 阶段 4：断开
client.__close______()
```
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：此时服务器未启动，8080 端口没有监听套接字（socket）；管道（TCP 连接）是基于套接字建立的，套接字不存在，管道也无法创建，客户端连接请求被拒绝。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server 套接字的作用是监听端口、接收连接请求（LISTEN 状态），属于 “监听套接字”；

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：端口号：网络层面标识主机上的服务（如 8080 端口对应实验服务器），解决 “数据跨网络找到目标应用” 的问题；描述符：主机内 / 进程内标识套接字资源的整数，解决 “进程区分自己管理的不同套接字” 的问题。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：代码中未指定客户端端口号；端口号由操作系统内核自动分配

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：HTTP/1.0 无 Keep-Alive（默认短连接），服务器返回响应后会主动关闭连接；客户端 recv() 检测到 “对方关闭连接”（返回空字节 b''），循环终止，程序退出。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：HTTP/1.1 默认开启 Connection: Keep-Alive（长连接），服务器返回响应后不会主动关闭连接，等待客户端后续请求；客户端 recv() 一直等待数据，因此卡住，连接状态保持 ESTABLISHED。
