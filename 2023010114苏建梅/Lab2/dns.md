...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS 的作用
DNS（域名系统）负责将人类易记的域名（如 www.yxnu.edu.cn）转换为机器可识别的 IP 地址（如 202.203.86.93），相当于互联网的“电话簿”。
访问网站时先进行 DNS 解析的原因
网络通信依赖 IP 地址进行路由和寻址，浏览器无法直接通过域名建立连接。因此必须先通过 DNS 解析获取目标服务器的 IP 地址，才能发起后续的 HTTP/HTTPS 请求，完成网站访问。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：**Transaction ID 是否一致**  
是的，请求包和响应包中的 **Transaction ID 均为 0x0b0b**，两者完全一致。Transaction ID（事务 ID）主要用于以下两方面：
DNS 使用 UDP 协议（无连接、不可靠），客户端通过 Transaction ID 将收到的响应与之前发出的请求进行一一对应，确保响应被正确匹配到等待中的查询。随机生成的 Transaction ID 可以增加 DNS 欺骗攻击的难度，攻击者若无法猜测正确的事务 ID，则难以伪造有效的响应包注入缓存。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：'A' 记录表示将域名解析为IPv4 地址，是访问网站时最常用的 DNS 记录类型。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS 默认使用 UDP 协议，端口号为 53。当响应报文过大超过 512 字节时，或进行主从 DNS 服务器之间的区域传输时，会改用 TCP 协议（端口 53）以保证数据的完整传输。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：`gethostbyname()` 不是现代工程中推荐使用的接口，主要原因有两点：一是它不支持 IPv6，只能解析 IPv4 地址，而现代网络环境需要同时兼容 IPv6。二是它不具备线程安全性，内部使用静态缓冲区，在多线程环境下容易引发数据竞争和崩溃，而现代编程更强调并发安全。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：如果一个域名返回了多个 IP 地址，通常说明该域名背后部署了多台服务器，用于实现负载均衡或高可用性。客户端通常会选择其中一个 IP 进行连接，当某台服务器故障或负载过高时，可以自动切换到其他可用 IP，从而提高服务的稳定性和响应速度。
